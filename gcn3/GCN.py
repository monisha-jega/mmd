...
class HREI():
	...
	def create_placeholder(self):
		...
		#
		#knowledge is [batch_size, num_tuples, knowledge_length]
		self.knowledge = tf.placeholder(tf.int32,[None, None, knowledge_length], name="knowledge") 
		#
		#Number of knowledge tuples for th context
		self.num_tuples = tf.placeholder(tf.int32, name="num_tuples")
		#
		...
		#
		#W2 is [gcn_layer1_out_size, gcn_layer2_out_size]
		self.W2 = tf.Variable(tf.random_normal([gcn_layer1_out_size, gcn_layer2_out_size], stddev=0.01), name = "W2")
		#
		#b2 is [gcn_layer2_out_size]
		self.b2 = tf.Variable(tf.random_normal([gcn_layer2_out_size], stddev=0.01), name = "b2")

		#MEMORY WEIGHTS
		#
		#memA is [knowledge_length, gcn_layer1_out_size]
		self.memA = tf.Variable(tf.random_normal([knowledge_length, gcn_layer1_out_size], stddev=0.01), name = "memA")
		#
		#memB is [knowledge_length, gcn_layer1_out_size]
		self.memB = tf.Variable(tf.random_normal([knowledge_length, gcn_layer1_out_size], stddev=0.01), name = "memB")
	...
	def gcn(self, encoder_text_outputs, encoder_image_states): 

		# Use encoder_text_outputs and encoder_image_states to make a graph of nodes (X) and edges (A)
		# encoder_text_outputs is max_context_len * [batch_size, max_utter_len, cell_state_size] tensors
		# encoder_image_states is max_context_len * [batch_size, num_images_in_context, cell_state_size] tensors
		text = tf.concat(encoder_text_outputs, axis = 1)
		images = tf.concat(encoder_image_states, axis = 1)
		# text is [batch_size, max_dialogue_len * max_utter_len, cell_state_size]
		...
		#gcn_layer1 is [batch_size, num_nodes, gcn_layer1_out_size]

		W2_batch = tf.stack([self.W2]*batch_size, axis = 0)
		b2_batch = tf.stack([tf.stack([self.b2]*num_nodes, axis = 0)]*batch_size, axis = 0)
		gcn_layer2 = tf.nn.relu(tf.add(tf.matmul(tf.matmul(self.A, gcn_layer1), W2_batch), b2_batch))
		#gcn_layer1 is [batch_size, num_nodes, gcn_layer2_out_size]
	...
	def memory(self, context_state):
		memA_batch = tf.stack(tf.stack([self.memA]* self.num_tuples, axis = 0) * batch_size, axis = 0)
		#memA is [batch_size, num_tuples, gcn_layer2_out_size, knowledge_length]
		memB_batch = tf.stack(tf.stack([self.memB]* self.num_tuples, axis = 0) * batch_size, axis = 0)
		#memB is [batch_size, num_tuples, gcn_layer2_out_size, knowledge_length]

		memory_codeA = tf.matmul(memB, self.knowledge)
		#memory_codeA is [batch_size, num_tuples, gcn_layer2_out_size]
		memory_codeB = tf.matmul(memA, self.knowledge)
		#memory_codeB is [batch_size, num_tuples, gcn_layer2_out_size]

		context_state_many = tf.stack([context_state] * self.num_tuples, axis = 1)
		#context_state_many is [batch_size, num_tuples, gcn_layer2_out_size]
		attention_context_product = tf.dot(context_state_many, memory_codeB)
		#attention_context_product is [batch_size, num_tuples]
		attention = tf.nn.softmax(attention_context_product)
		#attention is [batch_size, num_tuples]
		attention_stacked = tf.stack([attention]*gcn_layer1_out_size, axis = 2)
		#attention_stacked is [batch_size, num_tuples, gcn_layer1_out_size]
		
		knowledge_memory = memory_codeB * attention_stacked
		#knowledge_memory is [batch_size, gcn_layer2_out_size]
	...
	def hierarchical_encoder(self):		
		...
		for encoder_text_input in self.encoder_text_inputs:
			...
			encoder_text_outputs.append(encoder_text_output)
		# encoder_text_outputs is max_context_len * [batch_size, max_utter_len, cell_state_size] tensors
		...
		context_state = self.gcn(encoder_text_outputs, encoder_image_states)
		...
		#knowledge_state
		knowledge_state = self.memory(context_state)

		return final_state + knowledge_state
