...
def context_encoder(sentence_encodings):
	...
	with tf.variable_scope('enc2'):
		...
		#RNN cell converts [batch_size, max_context_len, cell_state_size] to [batch_size, max_context_len, cell_state_size] and returns
		# context_output: [batch_size, max_context_len, cell_state_size]
		# context_state: [batch_size, cell_state_size]
		(context_outputs_1, context_outputs_2), (context_state_1, context_state_2) = \
	...
	context_outputs_stack = tf.stack([context_outputs_1, context_outputs_2])
	context_outputs_mean = tf.reduce_mean(context_outputs_stack, 0)

	return context_outputs_mean, context_state_mean
...
def hierarchical_and_concat():
	...
	#hre_outputs is [batch_size, max_context_len, cell_state_size]
	hre_outputs, hre_state = context_encoder(sentence_encodings) 

	#text_attention_from_hre_ is [batch_size, max_context_len]
	text_attention_from_hre_ = tf.layers.dense(hre_state, max_context_len, name = "text_attention")
	text_attention_from_hre_prob = tf.nn.softmax(text_attention_from_hre_)
	text_attention_from_hre = tf.stack([text_attention_from_hre_prob] * cell_state_size, 2)

	#all_text_reps is [batch_size, max_context_len, cell_state_size]
	all_text_reps = tf.multiply(hre_outputs, text_attention_from_hre)
	#all_text_reps_attended is [batch_size, cell_state_size]
	all_text_reps_attended = tf.reduce_sum(all_text_reps, 1)

	#image_attention_from_hre_ is [batch_size, num_images_per_utterance]
	image_attention_from_hre_ = tf.layers.dense(hre_state, num_images_per_utterance, name = "image_attention")
	image_attention_from_hre_prob = tf.nn.softmax(image_attention_from_hre_)
	#image_attention_from_hre is [batch_size, num_images_per_utterance, cell_state_size]
	image_attention_from_hre = tf.stack([image_attention_from_hre_prob] * cell_state_size, 2)
	...
	#convert into [batch_size, num_images_per_utterance, cell_state_size]
	encoded_image_states_stacked = tf.stack(encoded_image_states, axis = 1)
	
	#all_image_reps is [batch_size, num_images_per_utterance, cell_state_size]
	all_image_reps = tf.multiply(encoded_image_states_stacked, image_attention_from_hre)
	all_image_reps_attended = tf.reduce_sum(all_image_reps, 1)

	#Concatenate
	#dialogue_slots_ph is num_features * [batch_size, feature_size]
	dialogue_slots_concat = tf.concat(dialogue_slots_ph, axis = 1)
	#dialogue_slots_concat is [batch_size, feature_size * num_features]

	final_features = tf.concat([hre_state, all_text_reps_attended, all_image_reps_attended, last_ds_ph], axis = 1)
