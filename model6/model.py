...
def hierarchical_and_concat():
	...
	sentence_encodings_stacked = tf.stack(sentence_encodings, axis=1)
	#sentence_encodings is now [batch_size, max_context_len, cell_state_size]
	sentence_encodings_mean = tf.reduce_mean(sentence_encodings_stacked, axis=1)
	#sentence_encodings_concat is now [batch_size, cell_state_size]
	...
	#encoded_image_states_stacked is now [batch_size, num_images_per_utterance, cell_state_size]
	encoded_image_states_stacked = tf.stack(encoded_image_states, axis = 1)
	#encoded_image_states_mean is now [batch_size, cell_state_size]
	encoded_image_states_mean = tf.reduce_mean(encoded_image_states_stacked, axis = 1)

	final_features = tf.concat([sentence_encodings_mean, encoded_image_states_mean, last_ds_ph], axis = 1)
